...
class Queue
{
	 ...
	 * get queue instance
	 * @param $queue queue
	 * @param $pconnect connect
	 */
	public static function instanc($queue='queue_00',$pconnect = true)
	{
		if(!self::$_self)
		{
			self::$_self = new self($queue,$pconnect);
		}
		return self::$_self;
	}
	
	private function __construct($queue,$pconnect =true)
	{
		//GET HOST AND PORT
		$usequeue = C($queue);
